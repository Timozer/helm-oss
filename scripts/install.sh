...
PROJECT_NAME="helm-oss"
PROJECT_GH="Timozer/$PROJECT_NAME"

set -e
set -u

if [ -n "${HELM_OSS_PLUGIN_NO_INSTALL_HOOK:-}" ]; then
...
on_exit() {
    ...
    if [ ${exit_code} -ne 0 ]; then
        echo "${PROJECT_NAME} install hook failed. Please remove the plugin using 'helm plugin remove oss' and install again." > /dev/stderr
    fi
    rm -rf "releases"
    exit ${exit_code}
}
